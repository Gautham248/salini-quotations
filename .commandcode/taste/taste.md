...
- When reviewing uncommitted changes, systematically diffs the working tree, reads changed source files, runs the existing test suite as a baseline, then proactively adds tests for any uncovered features. Confidence: 0.75
 Expects the agent to autonomously handle the full commit+push workflow when asked: stage the right files, run a final test gate, write a Conventional Commits message, and push — without intermediate clarification. Confidence: 0.75
